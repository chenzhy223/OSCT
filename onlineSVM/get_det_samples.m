...
function [feat, pos_samples, labels, weights]=get_det_samples(im, pos, window_sz, det_config, w2c, isgetsamples)
...
%% 检查检测区域大小是否合法
detector_wear_size = floor(window_sz*det_config.detector_win_ratio);
...
bb_samples = [xx(:), yy(:), ones(numel(xx),1)*t_sz(2), ones(numel(xx),1)*t_sz(1)];%二维矩阵
