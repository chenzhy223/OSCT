...
if( ~any(strcmp(shape,{'same', 'valid', 'full', 'smooth'})) )
    error( 'convnFast: unknown shape flag' );
end
...
sizB = size(B);
if (ndA>ndB)
    sizB = [sizB ones(1,ndA-ndB)];
end
if (ndA<ndB)
    sizA = [sizA ones(1,ndB-ndA)];
end
...
elseif( ndA==3 && ndB==2 )
    ...
    C = zeros( [size(C1), sizA(3)] );
    C(:,:,1) = C1;
    for i=2:sizA(3)
        C(:,:,i) = conv2( A(:,:,i), B, shape );
    end
    if (smoothFlag)
        for i=1:sizA(3)
...
sizfft = 2.^ceil(real(log2(sizA+sizB-1)));
psizfft=prod(sizfft);
...
for d=1:nd
    ...
    inds1{ d } = 1:radii(d);
    inds2 = inds;
    inds2{ d } = sizA(d)-radii(d)+1:sizA(d);
    Dind(inds1{:}) = 1;
    Dind(inds2{:}) = 1;
end
Dind = find( Dind );
...
elseif( nd==3 ) % accelerated for 3D arrays
    for i=1:nlocs
        ...
        za = sum(sum(sum(Apart.*Bpart)));
        zb=sum(sum(sum(Bpart)));
        vs(1,i) = za./zb;
    end
    ...
    for i=1:nlocs
        for d=1:nd
            extract{d} = Astarts(i,d):Aends(i,d);
        end
        Apart = A( extract{:} );
        for d=1:nd
            extract{d} = Bstarts(i,d):Bends(i,d);
        end
        Bpart = B( extract{:} );
        v = (Apart.*Bpart);
        vs(i) = sum(v(:)) ./ sum(Bpart(:));
